function [ A, MSE_A ,NRE_A, TIME_A] = IiSPALM_SGD(X,ops)
% ============== input =====================================
% X  : the data tensor
...
for it = 1:max_it 
    ...
    for d_update=1:dim
        ...
        Gra = ((A_h{d_update}*H'-X_sample')*H)/n_mb;
        % update the selected block
        %A{d_update} = proxr(A{d_update} - alpha*Gra+inertial*(A{d_update}-A_old{d_update}), ops, alpha, d_update);
        A_new{d_update} = proxr(A_t{d_update} - alpha*Gra, ops, alpha, d_update);  
    ...
    if mod(it,out_iter)==0
        ...
        disp(['TiSPALM-SGD at iteration ',num2str(mmm+1),' and the MSE is ',num2str(MSE_A(mmm+1))])
        disp(['TiSPALM-SGD at iteration ',num2str(mmm+1),' and the NRE is ',num2str(NRE_A(mmm+1))])
        disp('====')
        mmm = mmm + 1;
        a=tic;
    end  
